...
A block average cannot distinguish a method that holds target from one that over-covers and then collapses. Figure~\ref{fig:traces} plots rolling coverage through the test block. \todo{regenerate both figures from the realised-feedback artefacts; the numbers in this section are realised, the plotted curves are not yet}
...
\caption{Worst-cell coverage against calibration size, Electricity, fixed $K = 6$ grid. Baselines plateau; the conditional adaptive layer is data-limited, not structure-limited. \todo{regenerate from the realised-feedback artefact; add the static-conditional curve; remove the in-plot title}}
\label{fig:calwindow}
\end{figure}
